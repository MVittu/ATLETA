...
weighted_root <- file.path(root, "runs/03_population_weighting")
subgroup_root <- file.path(root, "runs/04_subgroups")
...
share_basis[any_carbon == 1, ] <- basis_spline(share[any_carbon == 1])
...
model <- rstan::stan_model(file = file.path(root, "weighted_model.stan"))
fit <- rstan::sampling(
  model,
  data = list(
...
diagnostics <- data.frame(
  model = "age_sex_poststratified", max_rhat = max(summary_matrix[, "Rhat"], na.rm = TRUE),
...
for (p in 1:2) {
  ...
  basis_values <- basis_spline(values)
...
fit_check <- do.call(rbind, lapply(1:2, function(p) {
  ...
  predicted <- vapply(seq_len(samples), function(s) {
    eta <- draws$alpha[s, p] +
      ...
      rowSums(sweep(share_basis[record_index, , drop = FALSE], 2L,
...
plot_conditional <- function(type, filename, title) {
  ...
  plot <- ggplot2::ggplot(plot_data, ggplot2::aes(value, fill = group)) +
    ...
    ggplot2::labs(
      title = title, subtitle = "Target-standardized Q75 vs Q25 carbon share among users",
  ...
  ggplot2::ggsave(file.path(root, "plots", filename), plot,
                  width = 9.5, height = if (type == "sex") 4.8 else 6,
                  dpi = 300, bg = "white")
}
plot_conditional("sex", "weighted_competition_dose_by_sex.png",
                 "Competition association conditional on sex")
plot_conditional("age", "weighted_competition_dose_by_age.png",
...
cat("Weighted and conditional multilevel analyses complete.\n")

functions {
  ...
  // w is meant to be sqrt(lambda_m), i.e. the eigen-frequency of the HSGP basis function,
  // not a raw Fourier frequency, so no extra 2*pi factor is applied here.
...
data {
  ...
  int<lower=0, upper=1> include_exposure;
...
model {
  ...
  for (n in 1:N) {
    ...
    real eta = alpha[period[n]] + X[n] * beta_common
      + discipline[n] * beta_discipline + athlete_effect[athlete[n]]
      + include_exposure * exposure;
...
generated quantities {
  ...
  for (n in 1:N) {
    ...
    real eta = alpha[period[n]] + X[n] * beta_common
      + discipline[n] * beta_discipline + athlete_effect[athlete[n]]
      + include_exposure * exposure;
    injury_rep[period[n]] += bernoulli_logit_rng(eta);
  }
}

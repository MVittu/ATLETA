...
# Based on: Baghestani, A. R., et al. (2019). Bayesian Critical Values for 
# Lawshe's Content Validity Ratio.

data_path <- "data/raw/content_validity/Content validity 2 SURVEY ATLETICA.csv"
...
cols_to_exclude <- c("Informazioni.cronologiche", 
...
if (ncol(items_data) != 2L) stop("Expected exactly two content-validity items")
colnames(items_data) <- c("Item_1_Elite_Athletes_Eligibility",
                          "Item_2_Professional_Assessment")
...
output_path <- file.path(output_dir, "CVR_Bayesian_Results_2.csv")
write.csv(results, output_path, row.names = FALSE)
cat("\nAnalysis complete, results saved to:", output_path, "\n")

# VISUALIZATION: CVR RESULTS SUMMARY
...
ggsave(filename = file.path(plot_dir, sprintf("CVR_analysis_2_%s.png", timestamp)), 
       plot = p_cvr_main, width = 12, height = 6, dpi = 300, bg = "white")

cat("Plot saved successfully with timestamp:", timestamp, "\n")
cat("  -", file.path(plot_dir, "CVR_analysis_2_*.png"), ": Bar chart of all items with validity status\n")
cat("\nItem naming: I1, I2, I3, ... correspond to survey items for easy reference.\n")

# SUMMARY: INVALID ITEMS TO REVIEW/REFUTE
